...
public class ApiKeyInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (provided != null && MessageDigest.isEqual(configuredKey.getBytes(StandardCharsets.UTF_8), provided.getBytes(StandardCharsets.UTF_8))) return true;
        response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
        response.setContentType(MediaType.APPLICATION_JSON_VALUE);
        String requestId = RequestCorrelationFilter.requestId(request);
        response.getWriter().write("{\"error\":\"unauthorized\",\"message\":\"A valid X-API-Key is required for write operations.\",\"requestId\":\"" + requestId + "\"}");
        return false;
    }
